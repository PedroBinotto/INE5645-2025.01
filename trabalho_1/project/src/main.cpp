...
void cook(int id, std::string name, int time_offset_millis, barrier &kitchen_barrier) {
  ...
  thread_safe_print("Cozinheiro " + name + " começou a trabalhar", cout_lock);
  ...
  while (is_open || !orders_received.empty()) {
    ...
    thread_safe_print("Cozinheiro " + name + " começou a preparar pedido " + std::to_string(current.id) +
                          " para cliente " + std::to_string(current.client_id),
                      cout_lock);
  }
}
...
void client(int id, int time_offset_millis) {
  ...
  thread_safe_print("Cliente " + std::to_string(id) + " aguardando pedido " + std::to_string(client_order.id),
                    cout_lock);
}
