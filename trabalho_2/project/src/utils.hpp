 ...
 */
template <typename T> inline std::string print_vec(const std::vector<T>& list, const std::string& sep = ", ")
{
    std::string msg = "[";
    for (size_t i = 0; i < list.size(); ++i)
        msg += std::format("{0} {1}", list[i], ((i == list.size() - 1) ? "]\n" : sep));
    return msg;
}
 ...
 */
inline program_args capture_args(int argc, const char** argv, bool verbose = false)
{
    switch (argc)
    {
    case 2:
    ...
    default:
        if (verbose)
        {
            ...
            print_vec(std::vector<std::string>(argv + 2, argv + argc));
            std::cout << "\033[31mDeve ser " << argv[0] << " <BLOCK_SIZE>\033[0m ou" << std::endl;
            std::cout << "\033[31mDeve ser " << argv[0] << " <BLOCK_SIZE, NUM_BLOCKS>\033[0m" << std::endl;
        }
 ...
 * Exits with error status if invalid input is passed in.
 */
inline void validate_args(program_args& args, int world_size, bool verbose = false)
{
    ...
    if (num_blocks > MAX_NUM_BLOCKS)
        fail(std::format("Limite máximo de número de blocos de memória alocados é {0}", MAX_NUM_BLOCKS));

    if (block_size > MAX_BLOCK_SIZE)
        fail(std::format("Limite máximo do tamanho dos blocos de memória alocados é {0}", MAX_BLOCK_SIZE));
}
 ...
 */
inline bool is_verbose(int world_rank) { return world_rank == MASTER_INSTANCE_ID; }
...
inline int resolve_maintainer(int world_size, int key) { return key % world_size; }
...
inline std::unique_ptr<uint8_t[]> encode_write_message(int key, block value)
{
    ...
    std::unique_ptr<uint8_t[]> message_buffer = std::make_unique<uint8_t[]>(total_size);
 ...
 * `[ int target_index {sizeof(int) bytes} ][ block data {BLOCK_SIZE bytes} ]`
 */
inline std::unique_ptr<WriteMessageBuffer> decode_write_message(std::unique_ptr<uint8_t[]> message_buffer)
{
    std::shared_ptr<GlobalRegistry> registry = GlobalRegistry::get_instance();
    int block_size = registry->get(GlobalRegistryIndex::BlockSize);
    ...
    return std::make_unique<WriteMessageBuffer>(WriteMessageBuffer(key, std::move(value)));
}
