...
GlobalRegistry::GlobalRegistry(int world_rank, int world_size, int num_blocks, int block_size, int timestamp) : data()
{
...
GlobalRegistry::get_instance(int world_rank, int world_size, int num_blocks, int block_size, int timestamp)
{
    if (instance.get() == nullptr)
    {
        instance = std::shared_ptr<GlobalRegistry>(
            new GlobalRegistry(world_rank, world_size, num_blocks, block_size, timestamp));
    }
    return instance;
};
